...
function saveChanges() {
    ...
    closeModal.call(document.getElementById("editModal"));
}

// Overload signatures
function saveNewProduct(): void;
function saveNewProduct(title: string, brand: string, price: number): void;

// Implementation
function saveNewProduct(title?: string, brand?: string, price?: number): void {
    if (title !== undefined && brand !== undefined && price !== undefined) {
        // Save a new product with the provided details
        fetch("https://dummyjson.com/products/add", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            ...
            .then(newProduct => {
                addProductHTML(newProduct);
                closeModal.call(document.getElementById("newProductModal"));
            })
            .catch(error => {
                console.error("Error adding the product:", error);
            });
    } else {
        // Handle the case where the form values are taken from the input fields
        const title = (document.getElementById("newTitle") as HTMLInputElement).value;
        const brand = (document.getElementById("newBrand") as HTMLInputElement).value;
        const price = +(document.getElementById("newPrice") as HTMLInputElement).value;
